...
window.addEventListener("DOMContentLoaded", async() => {
	...
	fetch("https://k1d-r.github.io/K1D_CLIENT_V2/news.html", {
				headers: {
			...
			.then(async(response, error) => {
				...
				for(let timeElement of news.getElementsByTagName("time")) {
					...
					if(todayYear == year && todayMonth == month) {
						if(todayDay == day) {
							timeElement.innerText = "오늘";
							continue;
						}
						else if(todayDay - 1 == day) {
							timeElement.innerText = "어제";
							continue;
						}
					}
	...
	function updateAccounts() {
		...
		addElement.innerHTML = `<img src="add.svg"/> <span>계정 추가하기</span>`;
		addElement.onclick = () => {
			main.style.display = null;
			login.style.display = "block";
	...
	ipcRenderer.on("msa", async(event, result) => {
		...
		if(msmc.errorCheck(result)) {
			...
			alert("로그인 할수 없음. 에러 코드 : " + result.type);
			return;
		}
	...
	async function play(server) {
		if(!launching) {
			...
			launchNote.innerText = "로그인을 재시도 하는 중...";
			try {
	...
	document.querySelector(".minecraft-folder").onclick = () => ipcRenderer.send("directory", "마인크래프트 폴더를 선택하세요", "minecraft");
	document.querySelector(".jre-location-change").onclick = () => ipcRenderer.send("directory", "JRE 폴더를 선택하세요r", "jre");

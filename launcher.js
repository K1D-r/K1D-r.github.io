...
class Launcher {
	...
	async launch(callback, progress, server) {
		...
		if(!fs.existsSync(versionJson)) {
			console.log("버전 정보를 다운로드 하는 중...");
			progress("버전 정보를 다운로드 하는 중...");
		...
		console.log("라이브러리를 다운로드 하는 중...");
		progress("라이브러리를 다운로드 하는 중...");
		...
		console.log("파일들을 다운로드 하는 중...");
		progress("파일들을 다운로드 하는 중...");
		...
		console.log("클라이언트를 다운로드 하는 중...");
		progress("클라이언트를 다운로드 하는 중...");

		await Version.downloadJar(version);

		console.log("JRE를 다운로드 하는 중...");
		progress("런타임을 다운로드 하는 중...");
		...
		console.log("디스코드 라이브러리를 준비 하는 중...");
		progress("디스코드 라이브러리를 다운로드 하는 중...");
		...
		process.on("exit", (code) => {
			if(code != 0) {
				console.error("게임이 오류와 함께 강제 종료 되었습니다. " + code);
				...
				console.log("게임이 강제 종료 되었습니다. ( 에러코드 : 0 )");
			}

async function run() {
	...
	ipcMain.on("crash", async(_event, report, file, optifine) => {
		let option = dialog.showMessageBoxSync(window, {
			title: "게임이 강제종료 되었습니다.",
			message: `게임에서 에러가 발생하여 게임이 강제종료 되었습니다.
게임의 버그를 빨리 고치기 위해서 깃허브에 버그를 제보 해주세요.`,
			type: "question",
			buttons: [
				"아무것도 하지 않기",
				"에러 기록 파일 열기",
...
[Game Log on Hastebin](${hasteUrl})`
		...
		let running = `Running K1D CLIENT v${Utils.version}`;
		...
		let url = new URL("https://github.com/K1D-r/K1D_CLIENT_V2/issues/new/")
	...
	ipcMain.on("quit", (event, result) => {
		if(result) {
			...
			if(dialog.showMessageBoxSync(window, {
						title: "* 경고 *",
						message: "런처를 종료하시면, 플레이 하고 있는 게임이 함께 종료 됩니다.",
						type: "question",
						buttons: [
							"종료하지 않기",
							"게임과 런처를 함께 종료하기"

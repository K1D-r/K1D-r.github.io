	...
	static update() {
		return new Promise(async(resolve) => {
			try {
				...
				console.log("업데이트를 확인 하는 중...");

				let latestRelease = (await axios.get("https://api.github.com/repos/K1D-r/K1D_CLIENT_V2/releases/latest")).data;

				if(latestRelease.name == currentVersion) {
					console.log("새로운 업데이트가 발견 되지 않음.");
					resolve(false);
					return;
				}
